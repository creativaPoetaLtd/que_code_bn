 ...
 */
export function sendNotification(app: Application, payload: NotificationPayload) {
    const io = app.get("io");
    if (!io) return;
 ...
 */
export function sendBulkNotification(app: Application, payloads: NotificationPayload[]) {
    const io = app.get("io");
    if (!io) return;
 ...
 */
export async function createAndSendNotification(app: Application, payload: NotificationPayload) {
 ...
 */
export async function markNotificationAsRead(app: Application, notificationId: string, userId: string) {
    const models = app.get('models') as ReturnType<typeof Models>;
    const notification = await models.Notification.findOne({ where: { id: notificationId, userId } });
    if (notification) {
