...
export const send_contact_invitation = async (
  ...
  try {
    ...
    const { inviteeId, profileUrl } = req.body;
    ...
    // If profileUrl is provided, extract the user ID from it
    if (profileUrl && !inviteeId) {
      const urlPattern = /\/welcome\/([a-f0-9\-]{36})/i;
      const match = profileUrl.match(urlPattern);

      if (!match || !match[1]) {
        res.status(400).json({ message: "Invalid profile URL format" });
        return;
      }
    ...
    if (!targetUserId) {
      res
        .status(400)
        .json({ message: "Either inviteeId or profileUrl is required" });
      return;
    }
    ...
    const existingContact = await read_function<ContactAttributes>(
      ...
      "findOne",
      {
        where: {
          [Op.or]: [
            { userAId: inviterId, userBId: targetUserId },
            { userAId: targetUserId, userBId: inviterId },
          ],
        },
      }
    );
    ...
    const existingInvitation = await read_function<ContactInvitationAttributes>(
      ...
      "findOne",
      {
        where: {
          [Op.or]: [
            { inviterId, inviteeId: targetUserId, status: "pending" },
            {
